...
@author: dewandaru@gmail.com
    ...
    This is Extended Geoparser code that combines event extractor with the spatial minimality disambiguation method.
...
HOW TO:
------
Run this piece of code in Spyder.    
...
if  '__init__' not in vars() and '__init__' not in globals():
    print("initialize BDG geoparser")
    #try:
    runfile('D:/Riset/Gatotkaca/Exp06-Extraction/ontology.py', wdir='D:/Riset/Gatotkaca/Exp06-Extraction')
    runfile('D:/Riset/Gatotkaca/Exp04-Multilabel/topicmerging.py', wdir='D:/Riset/Gatotkaca/Exp04-Multilabel')
    runfile('D:/Riset/Gatotkaca/Exp07-Word2Vec/word2vec_training.py', wdir='D:/Riset/Gatotkaca/Exp07-Word2Vec')
    runfile('D:/Riset/Gatotkaca/Exp-16-Geonames/geospatial_driver.py', wdir='D:/Riset/Gatotkaca/Exp-16-Geonames')
    __init__ = True
    #except:
    #    pass
...
def extract_arg(sent, i):
    ''' in the window of 4 words, extract argument according to the following extractor '''
...
ploc_options = 'event|entity|adm_level|arg|ismax' 
ploc_options = 'ismax|entity|event' # best so far is this combo
...
event_options = 'org_regex|arg_regex|ev_keywords|gaz|postag|entity'
...
def train_crf( cross_validation=False, skipFeatureBuilding=False, mini=False, rnd = 1, mode = "entity", max_iterations = 100, _c1 = 0.1, _c2 = 0.1, test_size = 0.2):
    ...
    if not skipFeatureBuilding or mode == "entity":
        if mini:
            sents = load_annotation("D:/Riset/Gatotkaca/Exp06-Extraction/train-test-v9/training-mini.txt", True)
        else:
            sents = load_annotation("D:/Riset/Gatotkaca/Exp06-Extraction/train-test-v9/training.txt", True)
    ...
    if mode == "event" :
        ...
        if 'entity' in event_options:
            ...
            for ix, sent in enumerate(y_pred_list["entity"]):
    ...
    if mode == "arg":
        ...
        if 'event' in arg_options: 
            ...
            print("merge the X_test's word.event feature using y_pred from previous train session")
            for ix, sent in enumerate(y_pred_list["event"]):
    ...
    if mode == "ploc":
        ...
        if 'arg' in ploc_options:           
            ...
            for ix, sent in enumerate(y_pred_list["arg"]):
...
def load_crf():
    global crf_entity, crf_event, crf_arg
...
def get_ambiguations():
    global sents
    sents = load_annotation("D:/Riset/Gatotkaca/Exp06-Extraction/train-test-v9/training.txt")
    toponym_idx = 0
    for sent in sents:
        #print ("---")
        startCollect = False
...
def disambiguate_document(toponyms):
    ''' toponyms are list of t in document, ex ['Jakarta', 'Jakarta Pusat', 'Pluit', 'Kuningan', 'Pluit'] '''
    ''' implement Leidner Spatial Minimality (2008) '''
    ...
    total = 1
    for t in toponyms:
        list_places = ambigus (t)
        if list_places:
            T.append( list_places )
            total = total * len(list_places)
            print ("x", len(list_places))
    ...
    for tupl in PROD:
        ...
        polygon_pts = list(set(polygon_pts)) # discard same ponts
    ...
    return winning
        #print (poly, poly.area, polygon_pts)
...
def disambiguate_document_sm(sents):
    ...
    for sent in sents:
        ...
        for w in sent:
            ...
            if startCollect:
                if w[2] == 'I-LOC' or w[2]=="B-LOC" and not EOL: #inside the LOC
                    toponym.append( w[0] )
                    if w[2]=="B-LOC":
                        widx_start = widx
...
def disambiguate_list_sm( noploc = True ):
    ''' disambiguating list based on spatial minimality .
    ...
    print ("disambiguating list of toponyms")
    sents = load_annotation("D:/Riset/Gatotkaca/Exp06-Extraction/train-test-v9/training.txt", noploc = noploc )
    ...
    for sent in sents:
        ...
        for w in sent:
            ...
            if w == "===" or w == "=": # end of document
                ...
                #toponyms = filter_duplicate_toponyms( toponyms ) 
                if len(toponyms) < 15: # cutoff due to exponential algorithm
                    winner = disambiguate_document( toponyms )
...
#https://megapolitan.kompas.com/read/2020/05/19/06424621/kali-ciliwung-meluap-17-rw-di-8-kelurahan-jakarta-terendam-banjir

train()

test = """Sebanyak 17 RW yang tersebar di delapan kelurahan di Jakarta terendam banjir pada Selasa (19/5/2020) pagi
